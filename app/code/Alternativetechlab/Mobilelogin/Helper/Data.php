...
use Alternativetechlab\Textlocal\Helper\Apicall as TextLocalApicall;

/**
 * Class Data
 * Alternativetechlab\Mobilelogin\Helper
 */
class Data extends AbstractHelper
{
    ...
    const PROMO_MOBILE = 'mobilelogin/promotionalapi/promotionalmobile';
    const PROMO_MSG = 'mobilelogin/promotionalapi/promotionalmessage';
    const PROMO_DLTID = 'mobilelogin/promotionalapi/promotionaldltid';
    /**
     ...
     * @param \Alternativetechlab\Mobilelogin\Model\OtpFactory $otp
    ...
    public function __construct(
        ...
        \Alternativetechlab\Mobilelogin\Model\OtpFactory $otp,
        \Alternativetechlab\Mobilelogin\Model\ResourceModel\Otp\CollectionFactory $otpCollection,
        ...
        TextLocalApicall $textLocalApiCall
    ) {
        ...
        $this->textLocalApiCall = $textLocalApiCall;
    }
    ...
    public function getApplicableCountry($isArray = true)
    {
        ...
        $country = array_merge($defaultCountry, explode(",", $this->getAllowCountry()));
        if (!$isArray) {
            $country = $this->jsonHelper->jsonEncode($country);
        }
    ...
    public function get_browser_name($userAgent)
    {
        ...
        // If the string starts with the string, strpos returns 0 (i.e., FALSE).
        // Do a ghetto hack and start with a space.
        ...
        if (strpos($t, 'opera')!==false || strpos($t, 'opr/')!==false) {
        ...
        } elseif (strpos($t, 'google')!==false) {  // Search Engines
            ...
            return '[Bot] MSN';
        } elseif (strpos($t, 'mj12bot')!==false) { // Common Tools and Bots
            return '[Bot] Majestic';
        ...
        } elseif (strpos($t, 'facebook')!==false) { // Miscellaneous
            return '[Bot] Facebook';
    ...
    public function sendUpdateOTPCode($data, $websiteId = 1)
    {
        try {
            if ($this->checkCustomerWithSameMobileNo($data, $websiteId)) {
                return ["status"=>false, "message"=>__(self::CUSTOMER_EXISTS)];
            }
    ...
    public function verifyRegistrationOTP($data, $websiteId = 1)
    {
        ...
        try {
            ...
            if (count($collection) == 1) {
                ...
                $result["message"] = __(self::INCORRECT_OTP);
            }
        } catch (\Exception $e) {
    ...
    public function sendLoginOTP($data, $websiteId = 1)
    {
        try {
            if (!$this->checkCustomerWithSameMobileNo($data['loginotpmob'], $websiteId)) {
                return ["status"=>false, "message"=>__(self::CUSTOMER_NOT_EXISTS)];
            }
    ...
    public function sendOTPCode($mobile, $websiteId = 1)
    {
        try {
            if ($this->checkCustomerWithSameMobileNo($mobile, $websiteId)) {
                return ["status"=>"exist", "message"=>__(self::CUSTOMER_EXISTS)];
            }
            ...
            return $this->callApiUrl($message, $mobile, $dlt);
        } catch (\Exception $e) {
            return ["status"=>false, "message"=>$e->getMessage()];
        }
    }
        ...
        try {
            ...
            return ["status"=>true, "message"=>
                ...
                you will receive an email with a link to reset your password.',
                    $this->escaper->escapeHtml($data['email'])
                )];
    ...
    public function resetForgotPassword($data, $websiteId = 1)
    {
        ...
        try {
            ...
            if (count($collection) == 1) {
                ...
                $result["message"] = __(self::CUSTOMER_NOT_EXISTS);
            }
        } catch (\Exception $e) {
            $result["message"] = $e->getMessage();
        }
    ...
    public function sendPromotionalAPI()
    {
        $promotionalMobiles = $this->scopeConfig->getValue(self::PROMO_MOBILE, ScopeInterface::SCOPE_STORE);
        $promotionalMessage = $this->scopeConfig->getValue(self::PROMO_MSG, ScopeInterface::SCOPE_STORE);
        $promotionalDlt = $this->scopeConfig->getValue(self::PROMO_DLTID, ScopeInterface::SCOPE_STORE);
        $promotionalMobiles = explode(",", $promotionalMobiles);
        foreach ($promotionalMobiles as $promotionalMobileVal) {
            $result = $this->textLocalApiCall
                ->callPromotionalApiUrl($promotionalMessage, $promotionalMobileVal, $promotionalDlt);
        }
        if (print_r($result)) {
            return ["status" => true, "message" => 'Promotional Message Sent !!!'];
        }
    }
}
